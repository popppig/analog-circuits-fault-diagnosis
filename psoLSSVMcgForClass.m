...
% fitness由准确率改为MSE，后又取消，因libsvm即为准确率 by KingsleyChu
% 2017.12.04
%% 参数初始化
if nargin == 2
...
indices = crossvalind('Kfold',testnum,K); %10为交叉验证折数
%% 产生初始粒子和速度
for i=1:pso_option.sizepop
    ...
    sum_fitness=0;
    for k = 1:K   %实验记进行10次(交叉验证折数)，求10次的平均值作为实验结果，
        test = (indices == k); train = ~test;  %产生测试集合训练集索引
        
        train_input=trainset(train,:);
        test_input=trainset(test,:);
        train_output=trainset_label(train,:);
        test_output=trainset_label(test,:);
        [Yc,codebook,old_codebook] = code(train_output,codefct);
        %[Yc_te,codebook_te,old_codebook_te] = code(test_output,'code_MOC');
        
        model=initlssvm(train_input,Yc,'c',gam,sig2,'RBF_kernel');
        model.kernel_pars=sig2;
        model=trainlssvm(model);
        Yd=simlssvm(model,test_input);
%         try
        predict_label = code(Yd,old_codebook,[],codebook);%解码分类结果
%         catch err
%             disp(k);
%         end
        testnum=size(test_output,1);
%     mse=sum(predict_label-trainset_label).^2/testnum;%MSE均方误差
%     rmse=sqrt(sum(predict_label-trainset_label).^2/testnum);%RMSE均方根误差
%         test_output = code(test_output,old_codebook,[],codebook);%解码分类结果
        right=sum(test_output==predict_label);
%     n = sum(trainset_label~=predict_label);
%     fitness(i) = (1-n/prod(size(trainset_label)))*100;%prod计算数组元素的连乘积,因label只有1列，即为元素个数
        acc(k) = (right/testnum)*100;%acc方便与libsvm对比
        disp(acc(k));
        sum_fitness = sum_fitness-acc(k);
    end
    fitness(i)=sum_fitness/K;
end

% 找极值和极值点
[global_fitness bestindex]=min(fitness); % 全局极值
local_fitness=fitness;   % 个体极值初始化
formatSpec = '初始化准确率为 %8.2f \n';
fprintf(formatSpec,-global_fitness);
...
%      model=trainlssvm(model); 
    ...
    formatSpec = '第%d次迭代后最优准确率为 %8.2f \n';
    fprintf(formatSpec,i,-global_fitness);
end
