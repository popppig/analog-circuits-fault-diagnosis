function [bestCVaccuarcy,bestc,bestg,hisAcc] = scaSVMcgForClass(train_label,train,sca_option)
...
%%佳哥改进,增加全局搜索速度
        if r4<0.5
            % Eq. (3.1)
            pop(j,:)= global_x+(r1*sin(r2)*abs(r3*global_x-pop(j,:)));
        else
            % Eq. (3.2)
            pop(j,:)= global_x+(r1*cos(r2)*abs(r3*global_x-pop(j,:)));
        end
%%
...
%         model = svmtrain(train_label, train, cmd_temp);
        ...
        if abs( fitness(j)-global_fitness )<=eps && pop(j,1) < global_x(1)%取较小的c
            global_x = pop(j,:);
            global_fitness = fitness(j);
        end
...
line1 = '适应度曲线Accuracy[SCAmethod]';
line2 = ['(终止代数=',num2str(sca_option.maxgen),',种群数量pop=', ...
    num2str(sca_option.sizepop),')'];
line3 = ['bestc=',num2str(bestc),'bestg=',num2str(bestg), ...
    'bestCVaccuarcy=',num2str(bestCVaccuarcy)];
